    ...
    uint8_t *quantA, uint8_t *quantB, float *quantC)
{
    ...
    */
    ...
    for (t = 0; t < M; t++) 
    {
        //gemm_nn(1, N, K, ALPHA, A + t*lda, lda, B, ldb, C + t*ldc, ldc, filter, input);
   
        remain = 4-(K%4);
        memcpy(filter, &quantA[t*lda], sizeof(uint8_t)*(K-(K%4)));  
        
        #pragma omp parallel for
        for(i=K-remain;i<K;i+=4)
        {
            virt_addr = filter + offset;
            partial = partial +(quantA[i]<<24);
            partial = partial +(quantA[i+1]<<16);
            partial = partial +(quantA[i+2]<<8);
            partial = partial +(quantA[i+3]);

            *(volatile uint32_t*)virt_addr = partial;
        }
        offset = 0x00;
        ...
        for (k = 0; k < K; k++) 
        {
            ...
            for(j=k*ldb+c_len*block;j<N;j++)
            {
                ...
                partial = partial +(quantB[i+1]<<16);
                partial = partial +(quantB[i+2]<<8);
                partial = partial +(quantB[i+3]);
                *(volatile uint32_t*)virt_addr = partial;
                offset += 0x04;
            }
        }
