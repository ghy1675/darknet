...
float read_SFR1(int fd, off_t target)
{
    void* map_base;
    void* virt_addr;
    float output;
...
void write_value(int fd, off_t target, float value)
{
	void *map_base;
	void *virt_addr;
	map_base = mmap(0, MAP_SIZE, PROT_WRITE | PROT_READ, MAP_SHARED, fd, target & ~MAP_MASK);
	virt_addr = map_base + (target & MAP_MASK);
	*(volatile float*)virt_addr = value;
	munmap(map_base, MAP_SIZE);
}
...
void gemm_nn(int M, int N, int K, float ALPHA,
    float *A, int lda,
    float *B, int ldb,
    float *C, int ldc, float *quantC, bool flag) // A = Filter(we ights)
{
    int i, j, k, q, l;
    int block = 4096;
    int count = 0;
    ...
    //if(flag)
    {
        ...
        for(i=0;i<K;i++)
        {
            ...
            *(volatile double*)virt_addr = A[i];
            //write_SFR(fd, target, A[i]);
       	    //write_value(fd, target , A[i]);
	    //printf("%lf %lf\n",A[i],read_SFR1(fd,target));
            offset += 0x08;
        }
        ...
        for (k = 0; k < K; ++k) 
        {
            ...
            for (j = 0; j < N; ++j) {
                ...
                *(volatile double*)virt_addr = B[k * ldb + j];
                offset += 0x08;
                //output = *((double*)virt_addr);
                //printf("%f %lf \n",B[k * ldb + j],output);
                if(offset == 0x18000)
                {
                    offset = 0x00;
                ...
                /*
                write_value(fd, target , B[k * ldb + j]);
                target += 0x08;
                if(target == MEM_BANK4)
                    target = MEM_BANK1;
	...
	//printf("%d %d %d\n",M,N/block,K);
		...
		//printf("i : %d\n",i);
                for(q=0;q<pivot;q++)
                {
                    for(j=0;j<12288;j++)
                    {
			//printf("%d %lf ",i*ldb+j+q*12288, B[i*ldb+j+q*12288]);
                        write_value(fd, target, B[i*ldb+j+q*12288]);
                        //printf("%lf\n",read_SFR1(fd,target));
			target += 0x08;
                    }
		    //printf("Quot : %X\n",target);
                ...
                for(j=q*12288;j<N;j++)
                {
                    write_value(fd, target, B[i*ldb+j]);
                    target = 0x08;
                }
		//printf("target address : %X\n",target);
                target = MEM_BANK7;
                for(k=q*12288;k < N-(q*12288);k++)
                {
                    quantC[k] = read_SFR1(fd, target);
                    target += 0x08;
                }
                ...
                //count = 0;
                /*
                for (j = 0; j < pivot*block*3; ++j) {
                    write_SFR(fd, target, B[i*ldb+j]);
                    target += 0x08;
                    if(j!=0 && j%12288 == 0)
                    {
                        // Place to enable Function 
                        target = MEM_BANK7;
                        for (i = 0; i < 100;  i++) {
                            tmp = read_SFR1(fd,target);
                            quantC[i] = tmp;
                            target += 0x08;
                        }
                        //count++;
                        if(j==(pivot*block*3-1))
                            break;
                        else
                            target = MEM_BANK1;
                    }
                }
                for(j=pivot*block*3;j<N;j++)
                {
                    write_SFR(fd, target, B[i*ldb+j]);
                    target += 0x08;
                }
        ...
        else
        {
            ...
            for (k = 0; k < K; ++k) 
            {
                ...
                target = MEM_BANK7;
                #pragma omp parallel for
                for (j = 0; j < (MEM_BANK8-MEM_BANK7)/8; ++j) {
                    tmp = read_SFR1(fd,target);
                    quantC[j] += tmp;
                    target += 0x08;
                }
            ...
            printf("Non block Address : %X\n",target);
            //target = MEM_BANK7; //real
    ...
    */
    ...
    //printf("flaggggggg\n");
    //gemm_nn_NPU(N,  K,  A,  lda,  B,  ldb,  C,  ldc,  quantC,  flag);
    //printf("flaggggggg\n");
